...
from scheduler.task_scheduler_planner_baseline_async import TaskSchedulerPlannerBaselineAsync
...
from settings.async_settings import GPU, EVAL_GPU, device, cpu_device, fl_parameters, settings, stochastic_settings, log_folder, log_path, train_log, stats_log
...
def get_tv_dset(path, type, train=True):
    ...
    if type == "cifar100":
        ...
        # Stanford Cars dataset unavailable on torchvision or official site, loading previous cached version
...
if __name__ == "__main__":
    ...
    fl_instance = BaselineParamLibrary(fl_parameters["users_pool"], lora_rank=central_rank, local_lora_rank=local_lora_rank, min_pool=settings['min_pool'], mode=mode, part_cnt=fl_parameters['part'], sync=False)
    ...
    task_scheduler = TaskSchedulerPlannerBaselineAsync(fl_parameters["users_pool"], fl_instance, plan)
