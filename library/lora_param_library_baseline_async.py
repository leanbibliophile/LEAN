...
# Implements FedFa https://arxiv.org/abs/2404.11015v2

class LoraParameterLibraryBaselineAsync():
    def __init__(self, rank, part_count=10, mode="avg", min_pool=0):
        self.part_count = part_count # Used as buffer for FedFa
        self.part = 0
        self.rank = rank
        self.id_to_param = {} 
        self.u_ids = set()
        ...
        self.window_length = min_pool

        # Mode can be "avg" or "stack"
        self.mode = mode
    ...
    def initialize(self, model_dict, layer_list):
        ...
        for l in layer_list:
            ...
            lora_b = f"{l}.lora_B" 
            self.id_to_param[l] = {}
            self.param_pool_ref[l] = deque()
            self.param_pool[l] = []

            in_feat, out_feat = LoraParameterLibraryBaselineAsync.extract_template(model_dict[lora_a], model_dict[lora_b])
    ...
    def _create_new_params(self):
        ...
        for i in range(self.rank):
            for p in self.lora_prefixes:
                ...
                index = self.counter
                indices[p].append(index)
                self.param_pool_ref[l].append(ParamPair(new_params['lora_A'], new_params['lora_B']))
                #self.param_pool_ref[p].append(ParamPair(l_a, l_b))
                self.param_pool[p].append(index)
            self.counter += 1
    ...
    def construct_tensor(self, id, state_dict):
        # Return local_state_dict regardless
        #if self.mode == "avg":
        if self.mode == "avg" or self.mode == "flora":
            ...
            for p in self.lora_prefixes:
                ...
                if self.mode == "flora":
                    ...
                    output_dict[lora_a] = self.rank_template[p].lora_A # Make sure for FLoRA the weights are fresh
    ...
    def checkout(self, id):
        ...
        if self.part == self.window_length:
    ...
    def checkin(self, id, state_dict):
        ...
        for p in self.lora_prefixes:
            lora_a = f"{p}.lora_A"
            lora_b = f"{p}.lora_B" 
            self.param_pool_ref[p].append(ParamPair(state_dict[lora_a], state_dict[lora_b]))
    ...
    def pop(self):
        for p in self.lora_prefixes:
            self.param_pool_ref[p].popleft()
        self.param_pool_len -= 1

    def synchronize(self):
        if self.mode == "stack":
            # Stacking doesnt really make sense with async? Cannot really stack with gradients
            for p in self.lora_prefixes:
                lora_A = f"{p}.lora_A"
                lora_B = f"{p}.lora_B"
                
                updates = list(self.param_pool_ref[p])
                A_list = [updates[idx]["A"] for idx in range(self.part_count)]
                B_list = [updates[idx]["B"] for idx in range(self.part_count)]
                index_hidden_layer = [[i for i in range(self.rank * idx, (self.rank+1) * idx)] for idx in range(len(updates))]

                update_tensor_by_update_lists_dim_0(self.local_state_dict[lora_A], A_list, index_hidden_layer)
                update_tensor_by_update_lists_dim_1(self.local_state_dict[lora_B], B_list, index_hidden_layer)
        if self.mode == "flora":
            state_dict_template = copy.deepcopy(self.local_state_dict)
            construct_index = torch.tensor([i for i in range(self.rank)])
            for p in self.lora_prefixes:
                    lora_A_agg = [v.A for v in self.param_pool_ref[p]] #[self.param_pool_ref[p][idx].A for idx in range(self.window_length)]
                    lora_B_agg = [v.B for v in self.param_pool_ref[p]]  #[self.param_pool_ref[p][idx].B for idx in range(self.window_length)]
                    lora_A = f"{p}.lora_A"
                    lora_B = f"{p}.lora_B"
            ...
            for p in self.lora_prefixes:
                ...
                lora_A_agg = [v.A for v in self.param_pool_ref[p]] 
                lora_B_agg = [v.B for v in self.param_pool_ref[p]]  
                #lora_A_agg = [self.param_pool_ref[p][idx].A for idx in range(self.window_length)]
                #lora_B_agg = [self.param_pool_ref[p][idx].B for idx in range(self.window_length)]

                #lora_A_agg = [updates[idx]["A"] for idx in range(self.part_count)]
                #lora_B_agg = [updates[idx]["B"] for idx in range(self.part_count)]
                
                #stack_A = torch.stack(lora_A_agg, dim=0)
                #stack_B = torch.stack(lora_B_agg, dim=1)
                avg_A = torch.sum(lora_A_agg, dim=0).div(self.part_count)
                avg_B = torch.sum(lora_B_agg, dim=0).div(self.part_count)
