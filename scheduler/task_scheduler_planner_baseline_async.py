...
import threading
from settings.async_settings import GPU, fl_parameters, settings, train_log, device, cpu_device, avail_gpu, LOGGER, FULL_RANK
...
class TaskSchedulerPlannerBaselineAsync:
                    ...
                    with self.client_inactive_cv[cur_id]:
                        ...
                        if self.checkedin % LOG_FREQ == 0:
                            ...
                            for entry in eval_logs:
                                ...
                                wandb.log({f"Valid/{use_stats_str}/rank_{FULL_RANK}/acc": acc, "Eval rank": e}, step=cur_checkins)
    ...
    def train(self, model, local_lr, pretrained_state_dict, dataset, u_id, fl_instance, assigned_gpu, local_rounds, base_batch_flops=None):
        with self.client_inactive_cv[u_id]: 
            ...
            with fl_instance.lock:
                ...
                checkout_state_dict = pretrained_state_dict # Just use instance of inserted state dict because we preloaded a snapshot of model at checkout
