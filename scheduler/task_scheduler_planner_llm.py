...
import threading
from settings.settings_llm import GPU, fl_parameters, settings, train_log, device, cpu_device, avail_gpu, LOGGER
        ...
        while execution_flag:
            ...
            if program_counter < len(self.plan):
                ...
                if op == "e":
                    ...
                    #with self.client_checkin_cv[cur_id]:
                    #    self.client_checkin_cv[cur_id].notify_all()
                    ...
                    with self.client_inactive_cv[cur_id]:
                        ...
                        with self.fl_instance.lock:
                            ...
                            print(f"{self.checkedin} total checkins")
                        ...
                        if self.checkedin % LOG_FREQ == 0:
                            ...
                            eval_logs = evaluate_llm(model_name, ground_truth, tokenizer, eval_device, valid_loader, self.fl_instance, e_rank_list)
                    
                            for entry in eval_logs:
                                e = entry["rank"]
                                acc = entry["accuracy"]

                                log_str = f"Eval Valid Epoch accuracy - rank {e}: {acc}"
                                print(log_str)
                                with open(train_log, "a") as log_file:
                                    log_file.write(log_str + "\n")
                                
                                wandb.log({f"Valid/rank_{e}/acc": acc, "Eval rank": e}, step=cur_checkins)
    ...
    def train(self, model, local_lr, pretrained_state_dict, dataset, u_id, fl_instance, assigned_gpu, local_rounds, base_batch_flops=None, test_data_sample=None):
        with self.client_inactive_cv[u_id]:
            ...
            with fl_instance.lock:
                ...
                local_model_sd = get_peft_model_state_dict(self.model)
                ...
                set_peft_model_state_dict(local_model, checkout_state_dict)
...
#                        prox_term += (w-w_t).norm(2)
                ...
                if counter % 25 == 0: 
                    log_str = f"Train accuracy {counter}, client {u_id}: loss: {round_loss}"
                    print(log_str)
            ...
            print(f"Train accuracy client {u_id}: loss: {round_loss}")
            ...
            checkin_params = get_peft_model_state_dict(local_model)
