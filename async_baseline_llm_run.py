...
from scheduler.task_scheduler_planner_baseline_async_llm import TaskSchedulerPlannerBaselineAsyncLLM
...
from settings.async_settings_llm import ALPHA, GPU, EVAL_GPU, device, cpu_device, fl_parameters, settings, stochastic_settings, log_folder, log_path, train_log, stats_log, FULL_RANK
...
if __name__ == "__main__":
    ...
    print("Init library")
    fl_instance = BaselineParamLibrary(fl_parameters["users_pool"], lora_rank=local_lora_rank, local_lora_rank=local_lora_rank, min_pool=fl_parameters["part"], mode=mode, part_cnt=fl_parameters['part'], sync=False, llm=True)

    lora_model = get_lora_model(model, local_lora_rank)
    lora.mark_only_lora_as_trainable(model)
    #model.load_state_dict(pretrained_state_dict, strict=False)
    central_model_dict = lora_model.state_dict() #get_peft_model_state_dict(lora_model)
    #lora_prefixes = {}
    ...
    task_scheduler = TaskSchedulerPlannerBaselineAsyncLLM(fl_parameters["users_pool"], fl_instance, plan, collator=collator)
    ...
    pretrained_state_dict = lora_model.state_dict()
